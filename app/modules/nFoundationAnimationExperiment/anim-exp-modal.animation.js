(function() {
	...
	function AnimExpModal($animateCss) {
		...
		function enter($element, done) {
			...
			$(innerEl).one('flipComplete', function() {
				_cleanUpStyles(innerEl);
				done();
			});
		...
		function leave($element, done) {
			...
			innerEl.classList.add('anim-exp-modal--out');
			...
			var FIRST = innerEl.getBoundingClientRect();
